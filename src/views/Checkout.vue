...
const totalCurrency = computed(() => siteCurrency.value || 'USD')
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 如果后端已返回支付链接（redirect 模式），直接跳转支付网关
    if (responseData.pay_url) {
      window.location.href = responseData.pay_url
      return
    }

    // 如果订单已通过余额全额支付，跳转到支付结果页
    if (responseData.order_paid) {

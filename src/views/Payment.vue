<template>
  <div class="min-h-screen theme-page pt-24 pb-16">
    <div class="container mx-auto px-4">
      <div v-if="showDiscordReturnPanel" class="mx-auto max-w-xl theme-panel rounded-2xl p-8 text-center">
        <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full theme-surface-soft text-2xl">✅</div>
        <h1 class="mb-2 text-2xl font-black theme-text-primary">Payment received</h1>
        <p class="theme-text-secondary text-sm leading-6">
          PayPal has returned successfully. You can close this page and go back to Discord. The bot is refreshing the order status automatically and will notify you after payment or delivery is confirmed.
        </p>
        <p class="mt-4 text-xs theme-text-muted">If the browser does not close automatically, please close this tab manually.</p>
        <button type="button" class="theme-btn-inline-md mt-6 border theme-btn-secondary font-semibold" @click="tryCloseDiscordReturnWindow">Close this page</button>
      </div>
      <template v-else>
...
const isDiscordReturn = computed(() => readRouteQueryValue('source').toLowerCase() === 'discord' || readRouteQueryFlag('discord_return'))
const showDiscordReturnPanel = computed(() => isDiscordReturn.value && paymentReturnMarkers.some(marker => readRouteQueryFlag(marker)))
...
const tryCloseDiscordReturnWindow = () => {
...
const scheduleDiscordReturnClose = () => {
  if (!showDiscordReturnPanel.value || typeof window === 'undefined') return
  window.setTimeout(() => {
    tryCloseDiscordReturnWindow()
  }, 1200)
  window.setTimeout(() => {
    tryCloseDiscordReturnWindow()
  }, 3500)
...
onMounted(() => {
  if (showDiscordReturnPanel.value) {
    loading.value = false
    scheduleDiscordReturnClose()
    return
  }

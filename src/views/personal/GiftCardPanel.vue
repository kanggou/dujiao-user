...
const redeemedAmountText = computed(() => {
  ...
  const currency = String(lastRedeem.value?.gift_card?.currency || appStore.config?.currency || 'USD').trim()
  if (!rawAmount) return '-'
  ...
  try {
    ...
    panelAlert.value = {
      ...
      message: t('personalCenter.giftCard.redeemSuccess', {
        ...
        currency: String(payload.gift_card?.currency || appStore.config?.currency || 'USD'),
      }),
    }
    redeemForm.code = ''
    resetCaptcha()
